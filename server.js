...
// Script durdur
app.post('/api/script/stop/:filename', (req, res) => {
    try {
        const filename = req.params.filename;
        // Windows'ta script process'ini durdur
        const ext = path.extname(filename).toLowerCase();
        
        if (ext === '.py') {
            // Python process'lerini durdur (filename ile ilişkili)
            exec(`taskkill /FI "WINDOWTITLE eq Python Script - ${filename}*" /F`, (error) => {
                if (error) {
                    console.log('Process zaten kapalı veya bulunamadı');
                }
            });
        } else if (ext === '.js') {
            // Node process'lerini durdur
            exec(`taskkill /FI "WINDOWTITLE eq JavaScript Script - ${filename}*" /F`, (error) => {
                if (error) {
                    console.log('Process zaten kapalı veya bulunamadı');
                }
            });
        ...
        res.json({ success: true, message: 'Script durduruldu' });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
